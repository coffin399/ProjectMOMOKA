...
`@<bot名>` でボットをメンションすると、AIが応答します。複数のAIモデルに対応しており、画像認識も可能です。
...
- Discord Bot Token
- 各種APIキー（使用する機能に応じて）
- （任意）音楽再生用: プロジェクト直下に Netscape 形式の `youtube_cookie.txt`
...
4. **ボットの起動**
   ...
   **手動起動 (Linux/Mac または手動で設定したい場合):**
   ```bash
   # Python 3.11 で仮想環境を作成（必須）
   py -3.11 -m venv .venv   # または python3.11 -m venv .venv
   ...
   旧 `.venv`（Python 3.10 等）がある場合は削除してから作り直してください。Windows では `startMOMOKA.bat` が自動で再作成します。
...
`@<bot名>`（例: `@MOMOKA`）でボットをメンションしてからメッセージを送信すると、AIが応答します。
...
4. プロジェクト直下に Netscape 形式の `youtube_cookie.txt` を配置する（または `config.yaml` の `music.youtube_cookie_file` を設定）。年齢制限・ボット判定対策に必要で、FFmpeg にも yt-dlp 由来の Cookie ヘッダーが渡されます。
5. **YouTube EJS（必須）**: yt-dlp が署名チャレンジを解くため、**Deno（推奨）または Node.js 22+** を PATH に入れてください。あわせて `pip install -U "yt-dlp[default]"` で `yt-dlp-ejs` を入れてください。未導入だと `Signature solving failed` / `Only images are available` になります。詳細: https://github.com/yt-dlp/yt-dlp/wiki/EJS
