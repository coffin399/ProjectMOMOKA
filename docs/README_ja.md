...
- 🔗 **Link Fix** - 壊れた SNS embed を Fix プロキシで代替表示（`/linkfix`・**PLANA 専用**）
...
公式 Discord embed が欠落・壊れているときだけ、SNS リンクを Fix 系プロキシ URL に差し替えて **silent 引用返信**します（動画プレビュー等が使えるようになります）。
...
| `link_fix_config.yaml` | Link Fix（壊れた SNS embed の代替・PLANA） |
...
| コマンド / 操作 | 説明 |
|----------------|------|
| SNS URL を投稿 | 公式 embed が無い／壊れているとき、Fix URL を silent 引用返信 |
...
- 対象 URL の公式 embed を短時間待ち、**メディア無し／不一致なら** Fix プロキシ URL を返信（常時は送らない）
- 返信は `@silent`。元メッセージは権限があれば embed 抑制
- Fix 側にも embed が付かない場合は返信を削除
